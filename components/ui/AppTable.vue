<template>
 ...
 :class="sortKey === column.key ? 'text-accent' : 'hover:text-text'"
 @click="handleSort(column.key)"
 >
 <span>{{ column.label }}</span>
 <span
 class="flex size-5 items-center justify-center rounded transition-all"
 :class="sortKey === column.key ? 'bg-accent/10 opacity-100' : 'opacity-0 group-hover:opacity-50'"
 >
 <AppIcon
 ...
 <AppIcon name="icon-[heroicons-outline--inbox-stack]" :size="1.75" class="text-text-muted/50" />
 </div>
 <div>
 <p class="text-text mb-0.5 text-sm font-medium">{{ emptyMessage }}</p>
 <p v-if="searchQuery" class="text-text-muted text-xs">Try adjusting your search term</p>
 </div>
 <AppButton
 v-if="searchQuery"
 variant="ghost"
 ...
 class="table-row-data border-border/40 border-b transition-colors last:border-b-0 hover:bg-accent/[0.03]"
 >
 <td
 v-for="column in visibleColumns"
 :key="column.key"
 ...
 :dark="false"
 >
 <span class="text-text-muted block max-w-[14rem] truncate text-start text-sm">
 ...
 :class="n === pageSize ? 'bg-accent/10 text-accent font-medium' : 'text-text hover:bg-muted'"
 ...
 <button
 type="button"
 class="border-border bg-surface text-text-muted hover:text-text hover:bg-muted flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-xs font-medium transition-colors"
 aria-haspopup="true"
 ...
 >
 <span v-if="columnSavedHint" class="text-success bg-success/10 flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-xs font-medium">
 ...
 :class="visibleColumnsSet.has(col.key) ? 'text-text hover:bg-muted/60' : 'text-text-muted/60 hover:bg-muted/40'"
...
const filteredData = computed(() => {
 ...
 return props.columns.some((col) => String(getValue(row, col.key)).toLowerCase().includes(q))
 })
 }
 if (sortKey.value) {
 result.sort((a, b) => {
...
tr:hover > .sticky-actions-td {
 background: color-mix(in srgb, var(--color-accent, #4f46e5) 3%, var(--color-surface, #fff));
}
