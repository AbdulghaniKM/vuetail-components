<template>
 ...
 class="flex w-full items-center gap-2 rounded-xl border bg-surface px-3.5 py-2.5 text-start text-sm transition-all focus:outline-none focus:ring-2 focus:ring-accent/20 disabled:cursor-not-allowed disabled:opacity-50"
 :class="[
 error ? 'border-error focus:border-error focus:ring-error/20' : 'border-border focus:border-accent',
 customClass,
 ]"
 @click="toggle"
 >
 <AppIcon name="icon-[heroicons-outline--calendar-days]" :size="1.125" class="text-text-muted shrink-0" />
 <span class="flex-1 truncate" :class="displayValue ? 'text-text' : 'text-text-muted'">
 ...
 v-if="isOpen"
 ref="dropdownRef"
 class="fixed z-[9998] w-[20rem] rounded-xl border border-border bg-surface shadow-xl"
 :style="dropdownStyle"
 >
 <!-- Header: month/year nav -->
 <div class="flex items-center justify-between border-b border-border px-4 py-3">
 <button type="button" class="flex size-8 items-center justify-center rounded-lg text-text-muted hover:bg-muted hover:text-text transition-colors" aria-label="Previous month" @click="prevMonth">
 ...
 class="appearance-none rounded-lg border border-border bg-surface py-1 pe-6 ps-2.5 text-sm font-semibold text-text cursor-pointer transition-all hover:bg-muted focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20"
 ...
 :size="0.75"
 class="text-text-muted pointer-events-none absolute end-1.5 top-1/2 -translate-y-1/2"
 />
 </div>
 <!-- Year select -->
 <div class="relative">
 ...
 <button type="button" class="flex size-8 items-center justify-center rounded-lg text-text-muted hover:bg-muted hover:text-text transition-colors" aria-label="Next month" @click="nextMonth">
 ...
 <span v-for="(day, i) in weekdayLabels" :key="i" class="text-center text-xs font-semibold uppercase tracking-tight text-text-muted">
 {{ day }}
 </span>
 </div>
...
//  Open / close 
function toggle() {
...
//  Month / year navigation 
...
function goToToday() {
 ...
 selectDay({ date: now, isCurrentMonth: true })}

//  Day selection 
function getDayClass(day: { isCurrentMonth: boolean; isToday: boolean; isSelected: boolean }) {
 if (day.isSelected) return 'bg-accent text-white font-medium hover:bg-accent/90'
 if (day.isToday) return 'bg-accent/10 text-accent font-medium hover:bg-accent/20'
 if (!day.isCurrentMonth) return 'text-text-muted/40'
...
//  Time updates 
