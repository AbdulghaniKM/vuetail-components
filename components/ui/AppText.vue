...
type Color = 'primary' | 'secondary' | 'accent' | 'text' | 'text-text-muted' | 'success' | 'warning' | 'error' | 'info';
...
const componentTag = computed(() => {
 return props.as || (variantMap[props.variant] || variantMap.span).tag;
});

// Size classes
const sizeClass = computed(() => {
 const variantDefaults = variantMap[props.variant] || variantMap.span;
...
const trackingClass = computed(() => {
 ...
 wide: 'tracking-tight',
 wider: 'tracking-tight',
 widest: 'tracking-tight',
 };
...
const computedStyles = computed(() => {
 ...
 if (props.color && !['primary', 'secondary', 'accent', 'text', 'text-text-muted', 'success', 'warning', 'error', 'info'].includes(props.color)) {
